...
predictor_labels=['UNEMPLOYMENT RATE',
 'CONSUMER PRICE INDEX',
 #'CNSTR',
 'TECH/IT SECTOR EMPLOYMENT',
...
pred=model.multistep_forecast(48)
model.plot_coeffs()
dfY1=model.predict(X_train.copy(),plot=True,Y_True=Y_train.copy(),plot_list=['RES_PROP_PRICE_INDX_APART'])
model.variable_importance(all_vars,predictor_labels)
#model.make_model()     
      


df_pred=pd.concat([dfY1,pred])

df_pred['PRPTY_PRICE_ACTUAL']=None
df_pred['PRPTY_PRICE_ACTUAL'].loc[Y.index]=Y['RES_PROP_PRICE_INDX_APART']
df_pred.to_csv('..//results//predicted.csv')
